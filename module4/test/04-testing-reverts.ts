...
describe('04-testing-reverts: Testing Error Handling', function () {
  ...
  it('should revert with correct error messages', async function () {
    ...
    await expect(cars.connect(otherUser).removeCar(1)).to.be.revertedWith(
      'Not owner',
    );

    // Test another error message
    await expect(cars.removeCar(999)).to.be.revertedWith('Car does not exist');
  });
  ...
  it('should handle multiple reverts correctly', async function () {
    ...
    await expect(cars.connect(otherUser).removeCar(2)).to.be.revertedWith(
      'Not owner',
    );
  });
});
