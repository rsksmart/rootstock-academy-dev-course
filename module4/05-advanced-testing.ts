...
describe("05-advanced-testing: Advanced Testing Techniques", function () {
  ...
  it("should allow unlock after time passes", async function () {
    ...
    await network.provider.send("evm_increaseTime", [3600]);  // ← FILL
    await network.provider.send("evm_mine");                 // ← FILL
  ...
  it("should use snapshot and revert", async function () {
    ...
    const snapshotId = await network.provider.send("evm_snapshot");  // ← FILL
    ...
    // Revert to snapshot
    await network.provider.send("evm_revert", [snapshotId]);  // ← FILL
  ...
  it("should manipulate balances for testing", async function () {
    ...
    await network.provider.send("hardhat_setBalance", [  // ← FILL
      testAccount.address,
